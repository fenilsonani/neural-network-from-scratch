"""Translation model using transformer architecture - version 2 with fixed parameters."""
...
class TranslationTransformer:
    ...
    def __init__(self, 
        ...
        for _ in range(n_layers):
            self.encoder_layers.append(TransformerBlock(d_model, n_heads, d_ff, dropout))
        
        # Decoder layers  
        self.decoder_layers = []
        for _ in range(n_layers):
            self.decoder_layers.append(TransformerDecoderBlock(d_model, n_heads, d_ff, dropout))
    ...
    def _collect_parameters_from_module(self, module, prefix='') -> Iterator[Parameter]:
        """Recursively collect parameters from a module."""
        # Get direct parameters
        if hasattr(module, 'parameters'):
            params = module.parameters()
            # If it returns a dict-like object with values() method
    ...
    def parameters(self) -> Iterator[Parameter]:
        """Get all model parameters as an iterator."""
        # Collect from embeddings
        yield from self._collect_parameters_from_module(self.src_embedding)
        yield from self._collect_parameters_from_module(self.tgt_embedding)
        
        # Collect from encoder layers
        for layer in self.encoder_layers:
            yield from self._collect_parameters_from_module(layer)
        
        # Collect from decoder layers
        for layer in self.decoder_layers:
            yield from self._collect_parameters_from_module(layer)
        
        # Collect from output projection
        yield from self._collect_parameters_from_module(self.output_projection)
