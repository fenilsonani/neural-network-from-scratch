...
class TestPushTo95:
    ...
    def test_adam_optimizer_complete(self):
        ...
        layer = Linear(4, 2)
        optimizer = Adam(
            layer.parameters(), lr=0.01, betas=(0.9, 0.999), eps=1e-8, weight_decay=0.01
        )
    ...
    def test_advanced_tensor_operations(self):
        ...
        if hasattr(result, "backward"):
            try:
                ...
                grad_output = np.ones_like(
                    scalar_loss.data if hasattr(scalar_loss, "data") else [1]
                )
    ...
    def test_numerical_stability_comprehensive(self):
        ...
        from neural_arch.functional.activation import relu, sigmoid, softmax, tanh
