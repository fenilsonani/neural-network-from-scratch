...
class TestTensorOperationPerformance:
    ...
    def test_tensor_creation_performance(self):
        ...
        for size in sizes:
            ...
            stats = benchmark.run_benchmark(lambda: Tensor(data, requires_grad=True), num_runs=100)

            print(
                f"Tensor creation {size}: {stats['mean_time']*1000:.2f}ms ± {stats['std_time']*1000:.2f}ms"
            )
    ...
    def test_matrix_multiplication_performance(self):
        ...
        for shape_a, shape_b in test_cases:
            ...
            stats = benchmark.run_benchmark(lambda: matmul(a, b), num_runs=20)

            print(
                f"MatMul {shape_a}x{shape_b}: {stats['mean_time']*1000:.2f}ms ± {stats['std_time']*1000:.2f}ms"
            )
    ...
    def test_gradient_computation_performance(self):
        ...
        print(
            f"Forward-Backward pass: {stats['mean_time']*1000:.2f}ms ± {stats['std_time']*1000:.2f}ms"
        )
    ...
    def test_softmax_performance_large_batches(self):
        ...
        for batch_size in batch_sizes:
            ...
            stats = benchmark.run_benchmark(lambda: softmax(x), num_runs=10)

            print(
                f"Softmax batch={batch_size}: {stats['mean_time']*1000:.2f}ms ± {stats['std_time']*1000:.2f}ms"
            )
...
class TestLayerPerformance:
    ...
    def test_linear_layer_performance(self):
        ...
        for in_features, out_features in layer_configs:
            ...
            # Forward pass benchmark
            stats = benchmark.run_benchmark(lambda: layer(x), num_runs=50)

            print(
                f"Linear {in_features}->{out_features}: {stats['mean_time']*1000:.2f}ms ± {stats['std_time']*1000:.2f}ms"
            )
    ...
    def test_embedding_layer_performance(self):
        ...
        for vocab_size, embed_dim in configs:
            ...
            stats = benchmark.run_benchmark(lambda: embedding(indices), num_runs=50)

            print(
                f"Embedding {vocab_size}x{embed_dim}: {stats['mean_time']*1000:.2f}ms ± {stats['std_time']*1000:.2f}ms"
            )
    ...
    def test_optimizer_performance(self):
        ...
        layers = [Linear(1000, 500), Linear(500, 200), Linear(200, 50), Linear(50, 10)]
...
class TestTrainingPerformance:
    ...
    def test_batch_size_scaling(self):
        ...
        for batch_size in batch_sizes:
            ...
            print(
                f"Batch size {batch_size}: {stats['mean_time']*1000:.2f}ms total, {time_per_sample*1000:.2f}ms per sample"
            )
...
class TestRegressionBenchmarks:
    ...
    def test_performance_regression(self):
        ...
        for test_name, current_time in results.items():
            ...
            print(
                f"{status} {test_name}: {current_time*1000:.2f}ms (baseline: {baseline*1000:.2f}ms)"
            )
