...
class TestExceptions:
    ...
    def test_tensor_error(self):
        ...
        with pytest.raises(TensorError) as exc_info:
            raise TensorError("Invalid tensor operation", 
                            context={"shape": (2, 3), "dtype": "float32"})
    ...
    def test_shape_error(self):
        ...
        with pytest.raises(ShapeError) as exc_info:
            raise ShapeError(
                "Shape mismatch", 
                context={"expected": (10, 20), "actual": (10, 30)}
            )
    ...
    def test_dtype_error(self):
        ...
        with pytest.raises(DTypeError) as exc_info:
            raise DTypeError(
                "Type mismatch",
                context={"expected": "float32", "actual": "int32"}
            )
    ...
    def test_device_error(self):
        ...
        with pytest.raises(DeviceError) as exc_info:
            raise DeviceError(
                ...
                context={"device": "cuda:0", "available": ["cpu", "mps"]}
            )
    ...
    def test_resource_error(self):
        ...
        with pytest.raises(ResourceError) as exc_info:
            raise ResourceError(
                "Out of memory",
                context={"required": "8GB", "available": "4GB", "device": "cuda:0"}
            )
    ...
    def test_data_error(self):
        ...
        with pytest.raises(DataError) as exc_info:
            raise DataError(
                "Invalid data format",
                context={"expected": "tensor", "actual": "list"}
            )
    ...
    def test_layer_error(self):
        ...
        with pytest.raises(LayerError) as exc_info:
            raise LayerError(
                ...
                context={"layer_type": "Linear", "input_size": 128}
            )
    ...
    def test_model_error(self):
        ...
        with pytest.raises(ModelError) as exc_info:
            raise ModelError(
                ...
                context={"model_type": "Transformer", "num_layers": 12}
            )
    ...
    def test_optimization_error(self):
        ...
        with pytest.raises(OptimizationError) as exc_info:
            raise OptimizationError(
                "Optimization failed",
                context={"algorithm": "Adam", "step": 1000}
            )
    ...
    def test_parameter_error(self):
        ...
        with pytest.raises(ParameterError) as exc_info:
            raise ParameterError(
                "Invalid parameter",
                context={"param_name": "learning_rate", "value": -0.1}
            )
    ...
    def test_exception_inheritance(self):
        ...
        exception_classes = [
            ...
            GradientError, OptimizerError, ConfigurationError,
            NumericalError, ResourceError, DataError, LayerError,
            ModelError, OptimizationError, ParameterError
        ]
