...
from neural_arch.models.registry import (
    ModelRegistry, ModelInfo, register_model, get_model, list_models
)
from neural_arch.models.utils import (
    ModelCard, get_weight_url, download_file, compute_file_hash,
    download_weights, load_pretrained_weights, save_weights,
    create_model_card_from_model, WEIGHTS_DIR
)
...
class MockModule(Module):
    ...
    def state_dict(self):
        return {
            'linear.weight': self.linear.weight.data,
            'linear.bias': self.linear.bias.data
        }
...
class TestModelRegistry:
    ...
    def test_registry_invalid_model_class(self):
        ...
        with pytest.raises(TypeError, match="must inherit from Module"):
            ModelRegistry.register(
                name="invalid_model",
                model_class=NotAModule,
                description="Invalid model class"
            )
    ...
    def test_registry_create_model_with_pretrained(self):
        ...
        pretrained_configs = {
            'default': {'num_classes': 1000, 'hidden_size': 256}
        }
    ...
    def test_registry_create_model_pretrained_errors(self):
        ...
        ModelRegistry.register(
            name="no_pretrained",
            model_class=MockModule,
            description="No pretrained configs"
        )
    ...
    def test_registry_list_models(self):
        ...
        ModelRegistry.register("model4", MockModule, "Model 4", 
                              pretrained_configs={'default': {}}, tags=["audio"])
    ...
    def test_download_weights_caching(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('neural_arch.models.utils.download_file') as mock_download:
                ...
                # Test force download
                result_path = download_weights("test_model", "config1", cache_dir, force_download=True)
    ...
    def test_load_pretrained_weights(self):
        ...
        try:
            ...
            assert not np.array_equal(
                new_model.linear.weight.data,
                original_model.linear.weight.data
            )
            ...
            np.testing.assert_array_equal(
                new_model.linear.bias.data,
                original_model.linear.bias.data
            )
...
class TestModelRegistryIntegration:
    ...
    def test_end_to_end_model_workflow(self):
        ...
        @register_model(
            ...
            pretrained_configs={
                'default': {'num_classes': 100, 'hidden_size': 128}
            },
            default_config='default',
