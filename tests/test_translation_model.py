...
import numpy as np
import pytest
from model_v2 import PositionalEncoding, TranslationTransformer
...
class TestTranslationTransformer:
    ...
    def test_init(self):
        ...
        model = TranslationTransformer(
            src_vocab_size=100, tgt_vocab_size=100, d_model=64, n_heads=4, n_layers=2, d_ff=128
        )
    ...
    def test_parameters(self):
        ...
        model = TranslationTransformer(
            src_vocab_size=50, tgt_vocab_size=50, d_model=32, n_heads=2, n_layers=1, d_ff=64
        )
    ...
    def test_masks(self):
        """Test mask creation."""
        model = TranslationTransformer(
            src_vocab_size=100, tgt_vocab_size=100, d_model=64, n_heads=4, n_layers=1
        )
    ...
    def test_generate(self):
        ...
        # Generate
        output = model.generate(src, max_length=10, sos_idx=1, eos_idx=2, temperature=1.0)
...
class TestIntegration:
    ...
    def test_small_training_step(self):
        ...
        # Create small model
        model = TranslationTransformer(
            src_vocab_size=20, tgt_vocab_size=20, d_model=32, n_heads=2, n_layers=1, d_ff=64
        )
        ...
        loss = cross_entropy_loss(
            Tensor(output_flat, requires_grad=True), Tensor(target_flat, requires_grad=False)
        )
