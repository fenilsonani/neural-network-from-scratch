...
from neural_arch.optimization.mixed_precision import (
    GradScaler, AutomaticMixedPrecision, MixedPrecisionManager,
    PrecisionConfig, cast_to_fp16, cast_to_fp32, is_fp16_safe_op,
    get_mixed_precision_manager, set_mixed_precision_config,
    is_autocast_enabled, autocast, mixed_precision_training
)
from neural_arch.optimization.fusion import (
    FusionEngine, FusionPattern, FusedOperation,
    LinearGELUFusion, LinearReLUFusion, ConvBNReLUFusion, LayerNormLinearFusion,
    get_fusion_engine, fuse_linear_activation, fuse_conv_bn_activation,
...
class TestAutomaticMixedPrecision:
    ...
    def test_amp_enabled_context(self):
        ...
        with patch('neural_arch.optimization.mixed_precision.get_default_dtype') as mock_get, \
             patch('neural_arch.optimization.mixed_precision.set_default_dtype') as mock_set:
    ...
    def test_amp_global_state(self):
        ...
        with patch('neural_arch.optimization.mixed_precision.get_default_dtype'), \
             patch('neural_arch.optimization.mixed_precision.set_default_dtype'):
...
class TestMixedPrecisionManager:
    ...
    def test_mp_manager_statistics(self):
        ...
        expected_keys = {
            "total_steps", "successful_steps", "skipped_steps",
            "success_rate", "current_scale", "enabled"
        }
    ...
    def test_mp_manager_state_dict(self):
        ...
        required_keys = {
            "scale", "growth_tracker", "step_count",
            "successful_steps", "skipped_steps", "config"
        }
...
class TestFusedOperations:
    ...
    def test_conv_bn_relu_fusion(self):
        ...
        try:
            ...
            expected_shape = (batch_size, out_channels, height - kernel_size + 1, width - kernel_size + 1)
...
class TestOptimizationIntegration:
    ...
    def test_context_managers(self):
        ...
        with mixed_precision_training(mock_model, mock_optimizer, enabled=True) as (scaler, autocast_ctx):
            assert scaler is not None
            assert autocast_ctx is not None
        
        # Test disabled case
        with mixed_precision_training(mock_model, mock_optimizer, enabled=False) as (scaler, autocast_ctx):
