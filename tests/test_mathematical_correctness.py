...
from neural_arch.functional import gelu, relu, sigmoid, tanh, softmax, mish, silu, swiglu
from neural_arch.functional import cross_entropy_loss, mse_loss
from neural_arch.nn import LayerNorm, BatchNorm1d, BatchNorm2d, RMSNorm, Linear
from neural_arch.functional.arithmetic import add, sub, mul, div, matmul
...
class TestMathematicalCorrectness:
    ...
    def test_gelu_gradient_correctness(self):
        ...
        for x_val in x_values:
            ...
            self.assert_gradients_close(
                x.grad, numerical_grad, rtol=1e-3, atol=1e-5
            )
    ...
    def test_cross_entropy_loss_correctness(self):
        ...
        expected_loss = -np.mean(np.log(softmax_probs[np.arange(batch_size), targets.data.astype(int)]))
...
class TestNumericalPrecision:
    ...
    def test_precision_consistency(self):
        ...
        np.testing.assert_allclose(
            y_f32.data.astype(np.float64), 
            y_f64.data, 
            rtol=1e-6  # Single precision tolerance
        )
