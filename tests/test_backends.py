...
class TestBackendOperations:
    ...
    def test_reduction_operations(self, backends):
        ...
        for backend in backends:
            ...
            assert np.allclose(
                backend.to_numpy(backend.sum(arr, axis=0)), 
                np.array([5.0, 7.0, 9.0])
            )
            assert np.allclose(
                backend.to_numpy(backend.sum(arr, axis=1)), 
                np.array([6.0, 15.0])
            )
            ...
            assert np.allclose(
                backend.to_numpy(backend.mean(arr, axis=0)), 
...
class TestBackendConsistency:
    ...
    def test_matmul_consistency(self):
        ...
        for size in SIZES[:2]:  # Test smaller sizes for consistency
            ...
            for backend1_name, backend2_name in pairs:
                ...
                assert np.allclose(c1_np, c2_np, rtol=1e-3, atol=1e-5), \
    ...
    def test_reduction_consistency(self):
        ...
        for backend1_name, backend2_name in pairs:
            ...
            for axis in [None, 0, 1, 2, (0, 1), (1, 2)]:
                ...
                assert np.allclose(sum1, sum2, rtol=1e-4, atol=1e-6), \
                    f"Sum mismatch on axis={axis}"
    ...
    def test_activation_consistency(self):
        ...
        for backend1_name, backend2_name in pairs:
            ...
            assert np.allclose(exp1, exp2, rtol=1e-4, atol=1e-6), \
            ...
            assert np.allclose(clip1, clip2, rtol=1e-4, atol=1e-6), \
            ...
            for backend_name, gpu_time in times.items():
                if backend_name != "numpy":
                    ...
                    assert gpu_time < cpu_time * 1.5, \
                        f"{backend_name} not faster than CPU"
