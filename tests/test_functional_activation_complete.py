...
class TestActivationFunctionsComplete:
    ...
    def test_activation_memory_efficiency(self):
        ...
        with patch("neural_arch.functional.activation.logger") as mock_logger:
            ...
            assert any(
                "Starting operation" in str(call) for call in mock_logger.debug.call_args_list
            )
            assert any(
                "Completed operation" in str(call) for call in mock_logger.debug.call_args_list
            )
