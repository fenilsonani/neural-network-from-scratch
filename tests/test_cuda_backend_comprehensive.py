    ...
    def test_successful_initialization_with_kernels(self):
        ...
        with patch("neural_arch.backends.cuda_backend.CUPY_AVAILABLE", True):
            with patch("neural_arch.backends.cuda_backend.CUDA_KERNELS_AVAILABLE", True):
                with patch("neural_arch.backends.cuda_backend.cp", self.mock_cp):
                    with patch(
                        "neural_arch.backends.cuda_backend.get_cuda_kernel_manager",
        ...
        with patch("neural_arch.backends.cuda_backend.CUPY_AVAILABLE", True):
            with patch("neural_arch.backends.cuda_backend.CUDA_KERNELS_AVAILABLE", True):
                with patch("neural_arch.backends.cuda_backend.cp", self.mock_cp):
                    with patch(
                        "neural_arch.backends.cuda_backend.get_cuda_kernel_manager",
                        side_effect=Exception("Kernel error"),
                    ):
                        with patch("builtins.print") as mock_print:
                            backend = CudaBackend()
                            mock_print.assert_called_with(
                                "⚠️ Failed to initialize custom CUDA kernels: Kernel error"
                            )
                            assert backend._kernel_manager is None
    ...
    def test_advanced_operations(self):
        ...
        with patch("neural_arch.backends.cuda_backend.CUPY_AVAILABLE", True):
            with patch("neural_arch.backends.cuda_backend.cp", self.mock_cp):
                ...
                self.mock_cp.einsum.assert_called_with(
                    "ij,jk->ik", self.mock_array, self.mock_array
                )

                # Test where
                backend.where(self.mock_array, self.mock_array, self.mock_array)
                self.mock_cp.where.assert_called_with(
    ...
    def test_layernorm(self):
        ...
        with patch("neural_arch.backends.cuda_backend.CUPY_AVAILABLE", True):
            with patch("neural_arch.backends.cuda_backend.cp", self.mock_cp):
                ...
                result = backend.layernorm(
                    self.mock_array, self.mock_array, self.mock_array, eps=1e-5
                )
            ...
            with patch("neural_arch.backends.cuda_backend.CUDA_KERNELS_AVAILABLE", True):
                with patch("neural_arch.backends.cuda_backend.cp", self.mock_cp):
                    with patch(
                        ...
                        assert result == 0.001
                        mock_kernel_manager.benchmark_kernel.assert_called_with(
                            "gelu", self.mock_array, num_runs=50
                        )
