...
class TestCrossEntropyLossComprehensive:
    ...
    def test_cross_entropy_basic_functionality(self):
        ...
        predictions_correct = Tensor(
            [[10, 1, 0.1]], requires_grad=True
        )  # Higher logit for correct class
...
class TestLossEdgeCases:
    ...
    def test_loss_with_nan_values(self):
        ...
        try:
            ...
            assert (
                not np.isfinite(loss_ce.data) or np.isnan(loss_ce.data) or np.isfinite(loss_ce.data)
            )
        except ValueError:
            # Tensor validation prevents NaN - acceptable behavior
            pass
