...
from neural_arch.functional.utils import (
    broadcast_tensors, reduce_gradient, get_broadcast_shape, validate_tensor_operation,
    ensure_tensor, compute_output_shape, check_finite_gradients, apply_gradient_clipping,
...
class TestActivationFunctionsCoverage:
    ...
    def test_all_activation_functions_basic(self):
        ...
        activations = [
            relu(x), sigmoid(x), tanh(x), gelu(x), gelu(x, approximate=True),
            mish(x), silu(x), leaky_relu(x), leaky_relu(x, negative_slope=0.2)
        ]
