...
from neural_arch.optim import (
    AdamW, StepLR, ExponentialLR, CosineAnnealingLR, LinearLR,
...
class TestLRSchedulers:
    ...
    def test_step_lr(self):
        ...
        for i, expected_lr in enumerate(expected_lrs):
            if i > 0:
                scheduler.step()
            assert abs(self.optimizer.lr - expected_lr) < 1e-6, f"Step {i}: expected {expected_lr}, got {self.optimizer.lr}"
    ...
    def test_cosine_annealing_lr(self):
        ...
        for epoch in range(T_max + 1):
            ...
            assert abs(self.optimizer.lr - expected_lr) < 1e-6, f"Epoch {epoch}: expected {expected_lr}, got {self.optimizer.lr}"
            scheduler.step()
    ...
    def test_linear_lr(self):
        ...
        end_factor = 1.0
        total_iters = 5
        scheduler = LinearLR(self.optimizer, start_factor=start_factor, end_factor=end_factor, total_iters=total_iters)
    ...
    def test_warmup_lr(self):
        ...
        warmup_epochs = 3
        warmup_factor = 0.1
        scheduler = WarmupLR(self.optimizer, warmup_epochs=warmup_epochs, warmup_factor=warmup_factor)
    ...
    def test_polynomial_lr(self):
        ...
        power = 2.0
        end_lr = 0.01
        scheduler = PolynomialLR(self.optimizer, total_iters=total_iters, power=power, end_lr=end_lr)
    ...
    def test_reduce_lr_on_plateau(self):
        ...
        scheduler = ReduceLROnPlateau(self.optimizer, mode='min', factor=0.5, patience=2, threshold=1e-4)
        ...
        for i, metric in enumerate(metrics):
            ...
            assert abs(self.optimizer.lr - initial_lr) < 1e-6, f"Step {i+1}: lr should not change with improving metrics"
    ...
    def test_convenience_functions(self):
        ...
        scheduler2 = get_cosine_schedule_with_warmup(self.optimizer, num_warmup_steps=2, num_training_steps=10)
