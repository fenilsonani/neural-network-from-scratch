...
from neural_arch.functional.activation import gelu, leaky_relu, relu, sigmoid, softmax, tanh
...
class TestActivationEdgeCases:
    ...
    def test_activation_with_inf_values(self):
        ...
        try:
            ...
            assert np.allclose(result_sigmoid.data[0, 0], 1.0, atol=1e-6) or np.isfinite(
                result_sigmoid.data[0, 0]
            )
            assert np.allclose(result_sigmoid.data[0, 1], 0.0, atol=1e-6) or np.isfinite(
                result_sigmoid.data[0, 1]
            )
        except ValueError:
            pass
        ...
        try:
            ...
            assert np.allclose(result_tanh.data[0, 0], 1.0, atol=1e-6) or np.isfinite(
                result_tanh.data[0, 0]
            )
            assert np.allclose(result_tanh.data[0, 1], -1.0, atol=1e-6) or np.isfinite(
                result_tanh.data[0, 1]
            )
        except ValueError:
            pass
    ...
    def test_activation_gradient_functions_exist(self):
        ...
        activations = [relu(x), sigmoid(x), tanh(x), gelu(x), leaky_relu(x), softmax(x)]
