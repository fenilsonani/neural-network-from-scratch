    ...
    def test_successful_initialization(self):
        ...
        with patch('neural_arch.backends.jit_backend.NUMBA_AVAILABLE', True):
            with patch('neural_arch.backends.jit_backend.logger') as mock_logger:
                ...
                mock_logger.info.assert_called_with("JIT backend initialized with Numba optimization")
        ...
        with patch('neural_arch.backends.jit_backend.NUMBA_AVAILABLE', False):
            with patch('neural_arch.backends.jit_backend.logger') as mock_logger:
                try:
                    ...
                    pass  # Expected
                mock_logger.warning.assert_called_with("Numba not available. Install with: pip install numba")
    ...
    def test_device_and_backend_utilities(self):
        ...
        with patch('neural_arch.backends.jit_backend.NUMBA_AVAILABLE', True):
            ...
            with patch('neural_arch.backends.jit_backend.logger') as mock_logger:
                ...
                mock_logger.warning.assert_called_with("JIT backend only supports CPU, ignoring device: cuda")
                assert result is x
