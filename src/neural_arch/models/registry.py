...
class ModelRegistry:
    ...
    def get_model_info(cls, name: str) -> ModelInfo:
        ...
        if name not in cls._models:
            ...
            raise ModelError(f"Model '{name}' not found. Available models: {available}")

        return cls._models[name]

    @classmethod
    def create_model(
        cls, name: str, pretrained: bool = False, config_name: Optional[str] = None, **kwargs
    ) -> Module:
        ...
        if pretrained:
            ...
            if not config_name:
                ...
                raise ModelError(f"No config specified. Available: {configs}")

            if config_name not in model_info.pretrained_configs:
                configs = list(model_info.pretrained_configs.keys())
                raise ModelError(f"Config '{config_name}' not found. Available: {configs}")
    ...
    @classmethod
    def list_models(
        cls, tags: Optional[List[str]] = None, with_pretrained: Optional[bool] = None
    ) -> List[str]:
    ...
    def save_registry(cls, path: Path) -> None:
        ...
        """
        registry_data = {"models": {}, "aliases": cls._aliases}
...
def get_model(
    name: str, pretrained: bool = False, config_name: Optional[str] = None, **kwargs
) -> Module:
    ...
    return ModelRegistry.create_model(
        name=name, pretrained=pretrained, config_name=config_name, **kwargs
    )


def list_models(
    tags: Optional[List[str]] = None, with_pretrained: Optional[bool] = None
) -> List[str]:
    """List available models.
