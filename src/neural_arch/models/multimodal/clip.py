...
from ..vision.vision_transformer import Block as ViTBlock
...
class MLP(Module):
    ...
    def gelu(self, x: Tensor) -> Tensor:
        ...
        x_data = (
            x.data * 0.5 * (1.0 + np.tanh(np.sqrt(2.0 / np.pi) * (x.data + 0.044715 * x.data**3)))
        )
...
class CLIPTextTransformer(Module):
    ...
    def __init__(
        ...
        for _ in range(layers):
            self.transformer.append(TextTransformerBlock(width, heads, dropout=dropout))
    ...
    def _init_weights(self):
        ...
        self.token_embedding.weight.data = (
            np.random.randn(*self.token_embedding.weight.shape).astype(np.float32) * std
        )

        # Initialize text projection with smaller scale
        self.text_projection.weight.data = np.random.randn(
            *self.text_projection.weight.shape
        ).astype(np.float32) * (std / np.sqrt(self.width))
...
class CLIP(Module):
    ...
    def __init__(
        ...
        if learnable_temperature:
            self.logit_scale = Parameter(np.array([np.log(1 / temperature_init)], dtype=np.float32))
        else:
