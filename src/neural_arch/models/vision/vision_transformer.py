...
from ...nn import Dropout, LayerNorm, Linear, MultiHeadAttention
...
class Block(Module):
    ...
    def __init__(
        ...
        if layer_scale_init is not None:
            self.layer_scale_1 = Parameter(np.ones(dim, dtype=np.float32) * layer_scale_init)
            self.layer_scale_2 = Parameter(np.ones(dim, dtype=np.float32) * layer_scale_init)
        else:
            self.layer_scale_1 = None
            self.layer_scale_2 = None

    def drop_path(self, x: Tensor, drop_prob: float = 0.0) -> Tensor:
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = Tensor(
            x.data + self.drop_path(y, self.drop_path_rate).data, requires_grad=x.requires_grad
        )
...
class VisionTransformer(Module):
    ...
    def _init_weights(self):
        ...
        if hasattr(self.patch_embed.proj, "weight"):
            ...
            std = 0.02
            self.patch_embed.proj.weight.data = (
                np.random.randn(*self.patch_embed.proj.weight.shape).astype(np.float32) * std
            )
...
def vit_l_32(**kwargs):
    ...
    return VisionTransformer(patch_size=32, embed_dim=1024, depth=24, num_heads=16, **kwargs)
