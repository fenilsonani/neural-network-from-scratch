...
class Conv2d(Module):
    ...
    def __init__(
        ...
        self.weight = Parameter(
            np.random.randn(out_channels, in_channels // groups, *self.kernel_size).astype(np.float32) * std
        )
            ...
            for oc in range(self.out_channels):
                for oh in range(out_height):
                    for ow in range(out_width):
                        ...
                        receptive_field = x_padded[b, :, h_start:h_start+self.kernel_size[0], w_start:w_start+self.kernel_size[1]]
...
class BasicBlock(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        out = Tensor(out.data + identity.data, requires_grad=out.requires_grad or identity.requires_grad)
        out = relu(out)
