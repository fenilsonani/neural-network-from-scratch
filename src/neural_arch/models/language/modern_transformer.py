...
from ...functional import gelu, matmul, softmax, swiglu
from ...nn import Dropout, LayerNorm, Linear, RMSNorm
from ...nn.positional import RoPE, RotaryPositionalEmbedding
...
class RoPEMultiHeadAttention(Module):
    ...
    def __init__(self, config: PreNormTransformerConfig):
        ...
        if config.use_rope:
            self.rope = RotaryPositionalEmbedding(
                dim=self.head_dim, max_seq_len=config.max_seq_len, base=config.rope_base
            )
        else:
            self.rope = None
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            masked_scores = np.where(mask_expanded, attention_scores.data, -1e9)
            attention_scores = Tensor(
                masked_scores,
                requires_grad=attention_scores.requires_grad,
...
class PreNormFeedForward(Module):
    ...
    def __init__(self, config: PreNormTransformerConfig):
        ...
        if config.activation == "swiglu":
            ...
            activation_func = (
                config.activation.lower() if config.activation.lower() in ["gelu", "relu"] else None
            )
            self.up_proj = Linear(
                config.d_model,
                config.d_ff,
...
class PreNormTransformerLayer(Module):
    ...
    def forward(
        self, x: Tensor, attention_mask: Optional[Tensor] = None, start_pos: int = 0
    ) -> Tensor:
...
class PreNormTransformer(Module):
    ...
    def _init_weights(self):
        ...
        # Token embeddings
        std = 0.02
        self.token_embedding.data = np.random.normal(0, std, self.token_embedding.shape).astype(
            np.float32
        )
