...
class BERTSelfAttention(Module):
    ...
    def transpose_for_scores(self, x: Tensor) -> Tensor:
        ...
        return Tensor(
            transposed_data, requires_grad=x.requires_grad, name=f"transposed({x.name or 'tensor'})"
        )
...
class BERTIntermediate(Module):
    ...
    def __init__(self, config: BERTConfig):
        ...
        self.dense = Linear(
            config.hidden_size, config.intermediate_size, activation="gelu", enable_fusion=True
        )
    ...
    def init_weights(self):
        ...
        for module in self.modules():
            if isinstance(module, Linear):
                ...
                module.weight.data = np.random.normal(0, std, module.weight.shape).astype(
                    np.float32
                )
...
BERTBase = bert_base
BERTLarge = lambda **kwargs: BERT(
    hidden_size=1024, num_hidden_layers=24, num_attention_heads=16, **kwargs
)
BERTModel = BERT
