...
class AdamW(Optimizer):
    ...
    def __init__(
        ...
        logger.info(f"Initialized AdamW optimizer: lr={lr}, beta1={beta1}, beta2={beta2}, weight_decay={weight_decay}")
        if lr_scheduler:
            logger.info(f"Learning rate scheduler: {lr_scheduler} with params {self.lr_scheduler_params}")
    ...
    def get_statistics(self) -> Dict:
        ...
        for name, param in self.parameters.items():
            ...
            if self.weight_decay > 0:
                weight_decay_effects.append(self.lr * self.weight_decay * np.linalg.norm(param.data))
