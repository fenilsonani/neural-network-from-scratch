        ...
        def backward_fn(grad_output: np.ndarray) -> None:
            ...
            for sample in range(batch_size):
                ...
                if gamma == 0.0:
                    ...
                    # ∂FL/∂p_t
                    dFL_dpt = alpha * (gamma * (one_minus_pt ** (gamma - 1)) * log_pt - (one_minus_pt ** gamma) / p_t)
                    ...
                    for class_idx in range(len(p_all)):
                        if class_idx != target_idx:
                            grad_predictions[sample, class_idx] = dFL_dpt * (-p_t * p_all[class_idx])
...
def label_smoothing_cross_entropy(
    ...
    if predictions.requires_grad:
        ...
        result._grad_fn = GradientFunction(backward_fn, [predictions, targets], "label_smoothing_cross_entropy")
...
def kl_divergence_loss(
    ...
    if predictions.requires_grad:
        ...
        result._grad_fn = GradientFunction(backward_fn, [predictions, targets], "kl_divergence_loss")
...
def cosine_embedding_loss(
    ...
    if input1.requires_grad or input2.requires_grad:
        ...
        result._grad_fn = GradientFunction(backward_fn, [input1, input2, target], "cosine_embedding_loss")
...
def triplet_loss(
    ...
    if not (anchor.shape == positive.shape == negative.shape):
        raise ValueError(f"All input shapes must match: {anchor.shape}, {positive.shape}, {negative.shape}")
    ...
    if anchor.requires_grad or positive.requires_grad or negative.requires_grad:
        ...
        result._grad_fn = GradientFunction(backward_fn, [anchor, positive, negative], "triplet_loss")
