...
class Tensor:
    ...
    def grad(self, value: Optional[np.ndarray]) -> None:
        ...
        if value_array.shape != self.shape:
            raise ValueError(
                f"Gradient shape {value_array.shape} doesn't match tensor shape {self.shape}")
    ...
    def backward(self, gradient: Optional[np.ndarray] = None) -> None:
        ...
        if gradient.shape != self.shape:
            raise ValueError(
                f"Gradient shape {gradient.shape} doesn't match tensor shape {self.shape}")
    ...
    def to(self, device: Optional[Union[Device, str]] = \
        None, dtype: Optional[Union[DType, str]] = None) -> 'Tensor':
    ...
    def numpy(self) -> np.ndarray:
        ...
        if self._requires_grad:
            raise RuntimeError(
                "Cannot convert tensor with requires_grad=True to numpy. Use detach() first.")
    ...
    def item(self) -> Union[float, int]:
        ...
        if self._data.size != 1:
            raise ValueError(
                f"Can only extract scalar from single-element tensor, got {self.size} elements")
    ...
    def sum(self, axis: Optional[Union[int, Tuple[int, ...]]] = \
        None, keepdims: bool = False) -> 'Tensor':
