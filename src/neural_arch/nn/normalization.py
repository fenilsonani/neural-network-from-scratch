...
class LayerNorm(Module):
    ...
    def __init__(
        ...
        if elementwise_affine:
            ...
            self.weight = Parameter(
                np.ones(normalized_shape, dtype=np.float32), name="layernorm.weight"
            )
            ...
            if bias:
                self.bias = Parameter(
                    np.zeros(normalized_shape, dtype=np.float32), name="layernorm.bias"
                )
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        requires_grad = x.requires_grad or (
            ...
            and (self.weight.requires_grad or (self.bias is not None and self.bias.requires_grad))
        )

        result = Tensor(
            output_data, requires_grad=requires_grad, name=f"layernorm({x.name or 'tensor'})"
        )
        ...
        if requires_grad:
            ...
            def backward_fn(grad_output: np.ndarray) -> None:
                ...
                if x.requires_grad:
                    ...
                    grad_mean = (
                        np.sum(grad_normalized * (-1.0 / std), axis=-1, keepdims=True)
                        + grad_var * np.sum(-2.0 * (x.data - mean), axis=-1, keepdims=True) / N
                    )
                ...
                if self.elementwise_affine and self.weight.requires_grad:
                    grad_weight = np.sum(
                        grad_output * normalized, axis=tuple(range(grad_output.ndim - 1))
                    )
                    ...
                    if self.weight._grad is None:
                        ...
                        self.weight._grad = self.weight._backend.add(
                            self.weight._grad, grad_backend
                        )
...
class RMSNorm(Module):
    ...
    def __init__(self, normalized_shape: int, eps: float = 1e-8, elementwise_affine: bool = True):
        """Initialize RMSNorm.
        ...
        if elementwise_affine:
            self.weight = Parameter(
                np.ones(normalized_shape, dtype=np.float32), name="rmsnorm.weight"
            )
        else:
            self.weight = None
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # Create result tensor
        requires_grad = x.requires_grad or (self.elementwise_affine and self.weight.requires_grad)

        result = Tensor(
            output_data, requires_grad=requires_grad, name=f"rmsnorm({x.name or 'tensor'})"
        )
        ...
        if requires_grad:
            ...
            def backward_fn(grad_output: np.ndarray) -> None:
                ...
                if x.requires_grad:
                    ...
                    rms_inv = 1.0 / rms
                    grad_rms = np.sum(
                        grad_normalized * normalized * (-rms_inv), axis=-1, keepdims=True
                    )
...
class BatchNorm1d(Module):
    ...
    def __init__(
        ...
        if affine:
            self.weight = Parameter(
                np.ones(num_features, dtype=np.float32), name="batchnorm1d.weight"
            )
            self.bias = Parameter(np.zeros(num_features, dtype=np.float32), name="batchnorm1d.bias")
        else:
            self.weight = None
            self.bias = None
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        result = Tensor(
            output_data, requires_grad=requires_grad, name=f"batchnorm1d({x.name or 'tensor'})"
        )
            ...
            def backward_fn(grad_output: np.ndarray) -> None:
                ...
                if self.affine and self.weight.requires_grad:
                    grad_weight = np.sum(
                        ...
                        axis=tuple(i for i in range(grad_output.ndim) if i != 1),
                    )
                ...
                if self.affine and self.bias.requires_grad:
                    grad_bias = np.sum(
                        grad_output, axis=tuple(i for i in range(grad_output.ndim) if i != 1)
                    )
        ...
        logger.debug(
            f"BatchNorm1d operation ({'train' if self.training else 'eval'}): {x.shape} -> {result.shape}"
        )
        return result
...
class BatchNorm2d(BatchNorm1d):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        result = Tensor(
            output_data, requires_grad=requires_grad, name=f"batchnorm2d({x.name or 'tensor'})"
        )
        ...
        logger.debug(
            f"BatchNorm2d operation ({'train' if self.training else 'eval'}): {x.shape} -> {result.shape}"
        )
        return result
...
class GroupNorm(Module):
    ...
    def __init__(self, num_groups: int, num_channels: int, eps: float = 1e-5, affine: bool = True):
        ...
        if affine:
            self.weight = Parameter(
                np.ones(num_channels, dtype=np.float32), name="groupnorm.weight"
            )
            self.bias = Parameter(np.zeros(num_channels, dtype=np.float32), name="groupnorm.bias")
        else:
            self.weight = None
            self.bias = None
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        result = Tensor(
            output_data, requires_grad=requires_grad, name=f"groupnorm({x.name or 'tensor'})"
        )
        ...
        if requires_grad:
            ...
            def backward_fn(grad_output: np.ndarray) -> None:
                ...
                grad_reshaped = grad_output.reshape(
                    N, self.num_groups, channels_per_group, *spatial_dims
                )
                normalized_reshaped = normalized.reshape(
                    N, self.num_groups, channels_per_group, *spatial_dims
                )
                ...
                if self.affine:
                    ...
                    grad_normalized_reshaped = grad_normalized.reshape(
                        N, self.num_groups, channels_per_group, *spatial_dims
                    )
                else:
                ...
                # Gradient w.r.t. input
                grad_var = np.sum(
                    grad_normalized_reshaped * (x_reshaped - mean), axis=norm_axes, keepdims=True
                )
                ...
                grad_mean = (
                    grad_mean
                    + grad_var
                    * np.sum(-2.0 * (x_reshaped - mean), axis=norm_axes, keepdims=True)
                    / group_size
                )

                grad_x_reshaped = (
                    grad_normalized_reshaped / std
                ...
                if self.affine:
                    if self.weight.requires_grad:
                        grad_weight = np.sum(
                            grad_output * normalized,
                            axis=tuple([0] + list(range(2, len(grad_output.shape)))),
                        )
                    ...
                    if self.bias.requires_grad:
                        grad_bias = np.sum(
                            grad_output, axis=tuple([0] + list(range(2, len(grad_output.shape))))
                        )
...
class InstanceNorm(Module):
    ...
    def __init__(
        ...
        if affine:
            self.weight = Parameter(
                np.ones(num_features, dtype=np.float32), name="instancenorm.weight"
            )
            self.bias = Parameter(
                np.zeros(num_features, dtype=np.float32), name="instancenorm.bias"
            )
        else:
            self.weight = None
            self.bias = None
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        result = Tensor(
            output_data, requires_grad=requires_grad, name=f"instancenorm({x.name or 'tensor'})"
        )
        ...
        if requires_grad:
            ...
            def backward_fn(grad_output: np.ndarray) -> None:
                ...
                grad_var = np.sum(
                    grad_normalized * (x.data - mean), axis=spatial_axes, keepdims=True
                )
                grad_var = grad_var * (-0.5) * (var + self.eps) ** (-1.5)
                ...
                grad_mean = (
                    grad_mean
                    + grad_var
                    * np.sum(-2.0 * (x.data - mean), axis=spatial_axes, keepdims=True)
                    / spatial_size
                )
                ...
                if self.affine:
                    if self.weight.requires_grad:
                        grad_weight = np.sum(
                            grad_output * normalized, axis=tuple([0] + list(spatial_axes))
                        )
                        self.weight.backward(grad_weight)
                        if hasattr(self.weight, "_backward"):
                            self.weight._backward()
