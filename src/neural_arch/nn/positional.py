...
class SinusoidalPositionalEncoding(Module):
    ...
    def __init__(
        self, d_model: int, max_len: int = 5000, dropout: float = 0.0, base: float = 10000.0
    ):
    ...
    def forward(self, x: Tensor, start_pos: int = 0) -> Tensor:
        ...
        if start_pos + seq_len > self.max_len:
            raise LayerError(
                f"Sequence length {start_pos + seq_len} exceeds max_len {self.max_len}"
            )
        ...
        result = Tensor(
            output_data, requires_grad=x.requires_grad, name=f"pos_enc({x.name or 'tensor'})"
        )
...
class RotaryPositionalEmbedding(Module):
    ...
    def __init__(
        self, dim: int, max_seq_len: int = 2048, base: float = 10000.0, precision: str = "float32"
    ):
        """Initialize RoPE.
    ...
    def forward(self, q: Tensor, k: Tensor, start_pos: int = 0) -> Tuple[Tensor, Tensor]:
        ...
        q_rotated = Tensor(
            q_rotated_data, requires_grad=requires_grad_q, name=f"rope_q({q.name or 'tensor'})"
        )

        k_rotated = Tensor(
            k_rotated_data, requires_grad=requires_grad_k, name=f"rope_k({k.name or 'tensor'})"
        )
        ...
        logger.debug(
            f"RoPE applied: q{q.shape} k{k.shape} -> rotated q{q_rotated.shape} k{k_rotated.shape}"
        )
...
class LearnedPositionalEmbedding(Module):
    ...
    def __init__(self, max_len: int, d_model: int, dropout: float = 0.0):
    ...
    def forward(self, x: Tensor, start_pos: int = 0) -> Tensor:
        ...
        result = Tensor(
            output_data, requires_grad=requires_grad, name=f"learned_pe({x.name or 'tensor'})"
        )
            ...
            def backward_fn(grad_output: np.ndarray) -> None:
                ...
                if self.embedding.requires_grad:
                    ...
                    if self.embedding._grad is None:
                        ...
                        self.embedding._grad = self.embedding._backend.add(
                            self.embedding._grad, grad_backend
                        )
