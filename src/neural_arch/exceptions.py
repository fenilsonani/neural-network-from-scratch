...
class NumericalError(TensorError):
    ...
    def __init__(
        ...
        message: str,
        operation: Optional[str] = None,
        invalid_values: Optional[str] = None
    ) -> None:
        context = {}
        if operation is not None:
            context["operation"] = operation
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            if "shape" in str(e).lower():
                raise ShapeError(
                    f"Shape error in {func.__name__}: {e}",
                ...
                raise NumericalError(
                    f"Numerical error in {func.__name__}: {e}",
                    operation=func.__name__
                ) from e
            ...
            raise ResourceError(
                f"Memory error in {func.__name__}: {e}",
