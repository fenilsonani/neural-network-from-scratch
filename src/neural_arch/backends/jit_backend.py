...
try:
    from numba import jit, cuda, types, prange
...
@jit(nopython=True, parallel=True, cache=True)
def jit_layernorm(x: np.ndarray, weight: np.ndarray, bias: np.ndarray, eps: float = 1e-5) -> np.ndarray:
...
@jit(nopython=True, parallel=True, cache=True)
def jit_conv2d(input_data: np.ndarray, weight: np.ndarray, bias: np.ndarray, 
    ...
    # Apply padding
    if padding > 0:
        padded_input = np.zeros((batch_size, in_channels, in_height + 2*padding, in_width + 2*padding))
        for b in prange(batch_size):
...
class JITBackend(Backend):
    ...
    def full(self, shape: Tuple[int, ...], fill_value: float, dtype: Optional[Any] = None) -> np.ndarray:
    ...
    def conv2d(self, input_data: np.ndarray, weight: np.ndarray, bias: np.ndarray,
              stride: int = 1, padding: int = 0) -> np.ndarray:
    ...
    def squeeze(self, x: np.ndarray, axis: Optional[Union[int, Tuple[int, ...]]] = None) -> np.ndarray:
    ...
    def split(self, x: np.ndarray, indices_or_sections: Union[int, List[int]], axis: int = 0) -> List[np.ndarray]:
        return np.split(x, indices_or_sections, axis=axis)
    
    def unique(self, x: np.ndarray, return_counts: bool = False) -> Union[np.ndarray, Tuple[np.ndarray, np.ndarray]]:
    ...
    def arange(self, start: float, stop: float, step: float = 1.0, dtype: Optional[Any] = None) -> np.ndarray:
        return np.arange(start, stop, step, dtype=dtype or np.float32)
    
    def random_normal(self, shape: Tuple[int, ...], mean: float = 0.0, std: float = 1.0, dtype: Optional[Any] = None) -> np.ndarray:
        result = np.random.normal(mean, std, shape)
        if dtype:
            result = result.astype(dtype)
        return result.astype(np.float32)
    
    def random_uniform(self, shape: Tuple[int, ...], low: float = 0.0, high: float = 1.0, dtype: Optional[Any] = None) -> np.ndarray:
