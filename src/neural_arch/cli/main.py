...
from ..config import get_preset_config, list_preset_configs, load_config
...
For more information, visit: https://github.com/your-org/neural-arch
    ...
    # Global arguments
    parser.add_argument("--version", action="version", version=f"Neural Architecture {__version__}")

    parser.add_argument("--config", type=Path, help="Configuration file path (.json or .yaml)")

    parser.add_argument(
        "--preset", choices=list_preset_configs(), help="Use a preset configuration"
    )
    ...
    train_parser.add_argument(
        "--data", type=Path, required=True, help="Path to training data directory or file"
    )
    train_parser.add_argument(
        "--output",
        type=Path,
    ...
    train_parser.add_argument(
        "--batch-size", type=int, help="Training batch size (overrides config)"
    )
    train_parser.add_argument(
        "--learning-rate", type=float, help="Learning rate (overrides config)"
    )
    train_parser.add_argument("--resume", type=Path, help="Resume training from checkpoint")
    train_parser.add_argument(
        "--validate", action="store_true", help="Run validation during training"
    )
    train_parser.add_argument(
    ...
    test_parser = subparsers.add_parser(
        "test", help="Test a trained model", description="Evaluate model performance on test data"
    )
    test_parser.add_argument("--model", type=Path, required=True, help="Path to trained model file")
    test_parser.add_argument(
        "--data", type=Path, required=True, help="Path to test data directory or file"
    )
    test_parser.add_argument("--output", type=Path, help="Output file for test results")
    test_parser.add_argument(
        "--metrics",
        nargs="+",
    ...
    test_parser.add_argument("--batch-size", type=int, help="Test batch size (overrides config)")
    ...
    benchmark_parser.add_argument(
        "--operations", nargs="+", default=["all"], help="Operations to benchmark (default: all)"
    )
    benchmark_parser.add_argument(
        "--sizes",
        nargs="+",
    ...
    benchmark_parser.add_argument("--output", type=Path, help="Output file for benchmark results")
    benchmark_parser.add_argument(
        "--format",
    ...
    info_parser.add_argument(
        "--performance", action="store_true", help="Show performance capabilities"
    )
    ...
    config_subparsers = config_parser.add_subparsers(
        dest="config_action", title="Config Actions", help="Configuration action to perform"
    )

    # Config create
    create_config_parser = config_subparsers.add_parser(
        "create", help="Create a new configuration file"
    )
    ...
    create_config_parser.add_argument(
        "--output", type=Path, required=True, help="Output configuration file path"
    )
    create_config_parser.add_argument(
        "--format",
        choices=["json", "yaml"],
    ...
    export_parser.add_argument(
        "--model", type=Path, required=True, help="Path to trained model file"
    )
    export_parser.add_argument(
        "--format",
