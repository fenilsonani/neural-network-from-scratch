...
class MemoryPool:
    ...
    def __init__(self, device: Device, max_pool_size: int = 1024 * 1024 * 1024):  # 1GB default
        ...
        self.available_blocks: Dict[DType, Dict[int, List[MemoryBlock]]] = defaultdict(lambda: defaultdict(list))
    ...
    def _cleanup_pool(self):
        ...
        logger.info(f"Pool cleanup: removed {blocks_cleaned} blocks, freed {bytes_cleaned / (1024**2):.1f} MB")
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            available_blocks_count = sum(
                len(blocks) for dtype_blocks in self.available_blocks.values()
    ...
    def get_pool(self, device: Device) -> MemoryPool:
        ...
        with self._lock:
            if device not in self.pools:
                ...
                logger.info(f"Created memory pool for {device} with size {pool_size / (1024**2):.0f} MB")
