...
from ..core.tensor import Tensor, GradientFunction, no_grad
...
class CheckpointFunction:
    ...
    def __init__(self, function: Callable, inputs: List[Tensor], 
                 outputs: List[Tensor], save_inputs: bool = True):
...
def memory_efficient_attention(query: Tensor, key: Tensor, value: Tensor, 
...
class CheckpointedTransformerLayer:
    ...
    def __init__(self, d_model: int, num_heads: int, d_ff: int, 
        ...
        self.feed_forward = SequentialCheckpoint(
            Linear(d_model, d_ff),
            lambda x: gelu(x),  # GELU activation
            Linear(d_ff, d_model)
        )
...
def estimate_memory_savings(model_layers: int, batch_size: int, 
                          seq_len: int, d_model: int) -> Dict[str, float]:
