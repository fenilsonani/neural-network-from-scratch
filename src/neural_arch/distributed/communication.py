...
class NCCLBackend(CommunicationBackend):
    ...
    def init_process_group(self, backend: str = "nccl", 
                          init_method: str = "env://", timeout: int = 1800):
        ...
        try:
            ...
            if init_method == "env://":
                ...
                self.nccl_comm = self.nccl.NcclCommunicator(
    ...
    def all_reduce(self, tensor: Tensor, op: ReduceOp = ReduceOp.SUM) -> Tensor:
        ...
        with self.cuda_stream:
            self.nccl_comm.allReduce(
                data.data.ptr, data.data.ptr, data.size,
                self._get_nccl_dtype(data.dtype), nccl_op,
        ...
        result = Tensor(data, requires_grad=tensor.requires_grad, 
    ...
    def all_gather(self, tensor: Tensor) -> List[Tensor]:
        ...
        with self.cuda_stream:
            self.nccl_comm.allGather(
                data.data.ptr, output_data.data.ptr, data.size,
                self._get_nccl_dtype(data.dtype), self.cuda_stream.ptr
            )
        ...
        for i in range(self.world_size):
            chunk = output_data[i]
            result_tensor = Tensor(chunk, requires_grad=tensor.requires_grad,
                                 dtype=tensor.dtype, device=tensor.device)
    ...
    def reduce_scatter(self, tensor: Tensor, op: ReduceOp = ReduceOp.SUM) -> Tensor:
        ...
        if data.shape[0] % self.world_size != 0:
            raise ValueError(f"Tensor size {data.shape[0]} not divisible by world size {self.world_size}")
        ...
        with self.cuda_stream:
            self.nccl_comm.reduceScatter(
                data.data.ptr, output_data.data.ptr, chunk_size,
                self._get_nccl_dtype(data.dtype), nccl_op,
                self.cuda_stream.ptr
            )
        ...
        result = Tensor(output_data, requires_grad=tensor.requires_grad,
                       dtype=tensor.dtype, device=tensor.device)
        return result
    ...
    def broadcast(self, tensor: Tensor, src: int) -> Tensor:
        ...
        with self.cuda_stream:
            self.nccl_comm.broadcast(
                data.data.ptr, data.data.ptr, data.size,
                self._get_nccl_dtype(data.dtype), src,
                self.cuda_stream.ptr
            )
...
class GlooBackend(CommunicationBackend):
    ...
    def init_process_group(self, backend: str = "gloo",
                          init_method: str = "env://", timeout: int = 1800):
    ...
    def all_gather(self, tensor: Tensor) -> List[Tensor]:
        ...
        for i in range(self.world_size):
            ...
            result_tensor = Tensor(data, requires_grad=tensor.requires_grad,
                                 dtype=tensor.dtype, device=tensor.device)
            results.append(result_tensor)
    ...
    def reduce_scatter(self, tensor: Tensor, op: ReduceOp = ReduceOp.SUM) -> Tensor:
        ...
        result = Tensor(chunk_data, requires_grad=tensor.requires_grad,
                       dtype=tensor.dtype, device=tensor.device)
        return result
...
def init_process_group(backend: str = "auto", init_method: str = "env://",
    ...
    logger.info(f"Distributed process group initialized: {backend} backend, "
